...
import { deletetSyrup, getSyrup } from "./api";
...
const Syruplist = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const syrups = await getSyrup();
        if (syrups.length > 0) {
          setProducts(syrups); // Set the products directly
        } else {
          setError("No products found.");
        }
      } catch (error) {
        console.error("Error fetching syrups:", error);
        setError("Error fetching syrups. Please try again.");
      } finally {
        setLoading(false);
      }
    };
...
try {
  ...
  const result=await deletetSyrup(id)
  ...
  return (
    <Container className="py-4">
      <h2 className="text-center mb-4">Syrups</h2>
        ...
        {products.length > 0 ? (
          products.map((product) => (
            <Col key={product._id} md={4} sm={6} lg={3} className="mb-4">
              ...
              <Card.Img
  variant="top"
  src={product.image}  // Corrected path if necessary
