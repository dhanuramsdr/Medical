...
import { addTablet,updateTablet } from './api';
...
console.log("id",id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await updateTablet(file, tabletData, id);
      console.log("Server Response:", result);

      if (result.success) {
        alert("Tablet updated successfully!");
        clearForm();
        navigate('/tablettlist');
      } else {
      ...
      <Row className="justify-content-center">
        <Col md={6}>
          <Card className="p-4 shadow">
            <Card.Body>
              <h2 className="text-center mb-4">Update Tablet</h2>
